  ...
  switch (handleReply.type) {
    ...
    case 'reply': {
      ...
      try {
        ...
        var getms = (await axios.get(data.download.HD, { responseType: "arraybuffer" })).data;
...
module.exports.run = async function ({ api, event, args, utils }) {
  ...
  if (args.join(" ").indexOf("https://") == 0) {
    var url = args.join(" ");
    var urlsplit = handleReply.link.split(/^.*(youtu.be\/|v\/|embed\/|watch\?|youtube.com\/user\/[^#]*#([^\/]*?\/)*)\??v?=?([^#\&\?]*).*/);
    ...
    messageID);
      ...
      try {
        ...
        return api.sendMessage('Đã xảy ra lỗi, vui lòng thử lại trong giây lát!!', event.threadID, event.messageID);
    }
  }
}